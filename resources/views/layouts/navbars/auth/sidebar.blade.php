<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 bg-gradient-primary" id="sidenav-main">
  ...
  <div class="collapse navbar-collapse h-auto" id="sidenav-collapse-main">
    <ul class="navbar-nav">
      ...
      <li class="nav-item">
        <a class="nav-link {{ (Request::is('unit-admins') ? 'active' : '') }}" href="{{ url('unit-admins') }}">
          <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
            <i style="font-size: 1rem;" class="fas fa-lg fa-user-cog ps-2 pe-2 text-center text-dark {{ (Request::is('unit-admins') ? 'text-white' : 'text-dark') }} " aria-hidden="true"></i>
          </div>
          <span class="nav-link-text ms-1 {{ (Request::is('unit-admins') ? 'text-dark' : 'text-white') }}">Unit Admins</span>
        </a>
      </li>
      <li class="nav-item">
